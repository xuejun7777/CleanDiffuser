...
class DiTBlock2(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        ...
        x = modulate2(self.norm1(x), shift_msa, scale_msa)

        x = x + gate_msa * self.attn(x, x, x)[0]
...
class FinalLayer1d2(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        shift, scale = self.adaLN_modulation(t).chunk(2, dim=2)
...
class DiT1d2(BaseNNDiffusion):
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock2(d_model, n_heads, dropout) for _ in range(depth)])
        self.final_layer = FinalLayer1d2(d_model, in_dim)
        self.initialize_weights()
    ...
    def forward(self,
        ...
        Input:
            x:          (b, horizon, in_dim)
            noise:      (b, )
            condition:  (b, horizon, emb_dim) or None / No condition indicates zeros((b, emb_dim))
